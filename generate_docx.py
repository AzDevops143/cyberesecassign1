...
def create_document():
    ...
    add_heading_1("3. Supply Chain Integrity & Cryptographic Signing")
    
    p = doc.add_paragraph()
    p.add_run("A critical aspect of the Hexa Force infrastructure is supply chain verification. To guarantee that only untampered, verified images are running in our production environments, the pipeline implements ")
    p.add_run("Cryptographic Container Signing").font.bold = True
    p.add_run(" using the standard ")
    p.add_run("Cosign").font.bold = True
    p.add_run(" tool from the Sigstore project.")
    
    p = doc.add_paragraph()
    p.add_run("Rather than relying on fragile private keys stored in GitHub Secrets, we employ ")
    p.add_run("Keyless OIDC (OpenID Connect) Signing").font.bold = True
    p.add_run(". GitHub Actions acts as an OIDC Identity Provider to issue a temporary JSON Web Token (JWT) containing cryptographic proof of the repository, workflow, commit SHA, and runner context. Sigstore's Certificate Authority (")
    p.add_run("Fulcio").font.bold = True
    p.add_run(") validates this token and issues a short-lived 10-minute certificate. The runner uses this certificate to sign the container image digest inside the registry, completely bypassing traditional key leakage and rotation risks.")

    add_callout(
        "By enforcing image signatures, runtime engines can mathematically verify that containers were built and verified strictly by our approved GitHub Actions pipeline before execution.",
        "COMPLIANCE NOTE", "0E7490", "F0F9FF"
    )

    # 4. Pipeline Integration
    add_heading_1("4. Pipeline Integration & Automation")
    ...
    p.add_run(", pushed to the ")
    p.add_run("GitHub Container Registry (GHCR)").font.bold = True
    p.add_run(", cryptographically signed keylessly using Cosign, and executed modularly. Below is the complete, validated YAML structure of the CI/CD pipeline, fully equipped with lowercase compliant repository tag parsing:")

    # GHA YAML block
    add_code_block([
        "name: Secure Build, Sign, and Run Pipeline",
        "",
        "on:",
        "  push:",
        "    branches: [ main ]",
        ...
        "    permissions:",
        "      contents: read",
        "      packages: write      # Required to push image and signature to GHCR",
        ...
        "",
        "    - name: Sign the Published Image keylessly",
        "      env:",
        "        TAGS: ${{ env.IMAGE_TAG }}:${{ github.sha }}",
        "      run: |",
        "        cosign sign --yes \"${TAGS}\"",
        "",
        "    - name: \"Run Stage 1 (Dirty COW Exploit)\"",
        "      run: docker run --rm ${{ env.IMAGE_TAG }}:latest /bin/bash run_demo.sh --stage-1",
        "",
        "    - name: \"Run Stage 2 (Capabilities Isolation)\"",
        "      run: docker run --rm ${{ env.IMAGE_TAG }}:latest /bin/bash run_demo.sh --stage-2"
    ])

    # 5. Mitigation Checklist
    add_heading_1("5. DevSecOps Hardening & Remediation Checklist")
    ...
    # Save
    output_path = "Hexa_Force_Secure_Lab_Guide.docx"
